...
PoCを超えた、より大きな規模のプロジェクトにおいては、コーディングツール（Claude Code / Codex / Gemini CLIなど）が十分にコンテキストを把握しきれず、不適切なコードを生成するケースがより多く発生します。その問題に対処するため、ドキュメントをしっかりと記述し、コーディングツールが必要に応じて適切なドキュメントを読み込んで最大限の効率を発揮できるようにしてあげるドキュメント駆動開発を試みます。そのテンプレートが本リポジトリです。

網羅的にドキュメント・項目を用意していますが、プロジェクトの目的・規模に応じて、取捨選択してください。
